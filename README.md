...
and the matching `oracle_output`. The one thing you have to tell it is
which task ID columns enumerate the targets that *share* a budget — here
the locations, so each model, reference date and horizon is a separate
allocation problem.
...
`allocate_model_out()` stops after the allocation, which is useful when
the question is what a forecast implies you should do rather than how
good it was:
...
given as any parametric family, with `add_pdqr_funs()` building the cdfs
and quantile functions from parameter columns:
...
target. The `stdize_news_params()`, `stdize_ou_params()` and
`stdize_met_params()` helpers convert the newsvendor, over/under-cost
and cost-loss parameterizations into `kappa` and `alpha`.
...
(2009), which `allocate()` reproduces. See `vignette("zxh-newsvendor")`.
